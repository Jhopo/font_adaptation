...
class UNetGenerator(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=3, num_downs=8, ngf=64, embedding_num=40, embedding_dim=128, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        #self.embedder.weight.requires_grad = False # Add: Freeze the ininial weight

    def forward(self, x, style_encoding):
        """Standard forward"""
        return self.model(x, style_encoding)
...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def forward(self, x, style=None):
        if self.outermost: # The outer block
            enc = self.down(x)
            if style is None:
            ...
            dec = self.up(sub)
            return dec, encode
        elif self.innermost: # No submodule
            encode = self.down(x)
            if style is None:
                return encode
            style = style.view(style.shape[0], style.shape[1], 1, 1)
            enc = torch.cat([style, encode], 1)
            dec = self.up(enc)
